...
(>>>) ::
  ...
  -> Parser b
-- (>>>) = lift2(flip const)
(>>>) pa pb = P(\i -> onResult(parse pa i) (\j _ -> parse pb j))
...
-- listtofp = p1 :. p2 :. p3 :. Nil
-- sequenceParser p1 p2 p3 = p1 `lift2cons` (p2 `lift2cons` (p3 `lift2cons` pure Nil))
